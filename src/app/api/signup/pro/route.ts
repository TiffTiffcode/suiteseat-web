...
export async function POST(req: NextRequest) {
  ...
  const doc = await AuthUser.create({ firstName, lastName, email, password: hash, role: 'pro' });
  const cookieStore = await cookies();
  cookieStore.set('session', String(doc._id), { httpOnly:true, path:'/', maxAge:60*60*24*7, sameSite:'lax' });
  return NextResponse.json({ ok:true, user:{ id:String(doc._id), firstName, lastName, email, role:'pro' } });
}
