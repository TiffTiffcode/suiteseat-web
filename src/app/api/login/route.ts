...
export async function POST(req: NextRequest) {
  try {
    ...
    // ---- password check (handles new + legacy) ----
    const stored = (user.passwordHash || user.password || '').trim();
    let valid = false;

    if (stored.startsWith('$2')) {
      // bcrypt hash (either in passwordHash or legacy password)
      valid = await bcrypt.compare(String(password), stored);
    } else if (stored) {
      // legacy plain-text password
      valid = String(password) === stored;

      // if it matched, upgrade to bcrypt hash and store in passwordHash
      if (valid) {
    ...
    // ---- set session cookie (note the await) ----
    const cookieStore = await cookies();
    cookieStore.set('session', String(user._id), {
      httpOnly: true,
      sameSite: 'lax',
      secure: true,       // ok since you’re on https in prod
      path: '/',
