...
function lookupService(id: string) {
  ...
  if (isConfirmOpen) {
    _pending = () => { setConfirmStage("book"); setIsConfirmOpen(true); };
  }
  setIsAuthOpen(true);
}
...
async function createAppointment() {
  ...
  try {
    ...
    const knownById = new Map((services || []).map((s: any) => [String(s._id), s]));
    const missingIds = serviceIds.filter(id => !knownById.has(String(id)));

    let fetched: any[] = [];
    if (missingIds.length) {
      try { fetched = await fetchServicesByIds(missingIds); } catch {}
    }
    const allPicked: any[] = serviceIds
      .map(id => knownById.get(String(id)) || fetched.find(s => String(s._id) === String(id)))
      .filter(Boolean);

    let combinedMin = serviceDurationMin || 0;
    const recomputedMin = allPicked.reduce((acc, s) => acc + readMinFlexible(s), 0);
    if (Number.isFinite(recomputedMin) && recomputedMin > 0) combinedMin = recomputedMin;
    ...
    // Build the ACTUAL values we send
 // Build the record payload (dynamic Record API)
// ✅ Values to POST — nested refs + denormalized ids + synonyms
const values: any = {
  // refs (objects)
  Business: businessId ? { _id: businessId } : undefined,
  Calendar: selectedCalendarId ? { _id: selectedCalendarId } : undefined,
  Client:   currentUserId ? { _id: currentUserId } : undefined,

  // denormalized ids (strings) so public/records can match any shape
  businessId:        businessId || undefined,
  calendarId:        selectedCalendarId || undefined,
  clientId:          currentUserId || undefined,
  BusinessId:        businessId || undefined,        // legacy/alt
  CalendarId:        selectedCalendarId || undefined,
  ClientId:          currentUserId || undefined,

  // date/time
  Date:       selectedDateISO,                       // "YYYY-MM-DD"
  StartTime:  selectedTimeHHMM,                      // canonical
  EndTime:    addMinutesHHMM(selectedTimeHHMM, combinedMin),

  // duration (multiple names for old readers)
  DurationMin:       combinedMin,
  Duration:          combinedMin,
  "Duration (min)":  combinedMin,

  // synonyms some UIs still read
  ...
  // services: always an array of refs
  "Service(s)": serviceIds.map(id => ({ _id: id })),

  // let server also lift these if it wants
...
const res = await fetch(`${API}/api/records/Appointment`, {
  ...
  body: JSON.stringify(values),
    ...
    if (!res.ok) {
      console.warn("[createAppointment] failed", await res.text());
      return false;
    }

    const saved = await res.json().catch(() => null);
    console.debug("[createAppointment] server response:", saved);

    // quick verify pull
    try {
      const checkUrl = `${API}/public/records?dataType=Appointment&Calendar=${encodeURIComponent(selectedCalendarId!)}&Date=${encodeURIComponent(selectedDateISO!)}&ts=${Date.now()}`;
      const checkRes = await fetch(checkUrl, { credentials: "include" });
      const checkRows = checkRes.ok ? await checkRes.json() : [];
      console.debug("[verify] appointments for day:", {
        count: Array.isArray(checkRows) ? checkRows.length : 0,
        sample: Array.isArray(checkRows) ? checkRows.slice(0, 3) : checkRows,
      });
    } catch {}

    // optimistic remove + refresh availability
    ...
    // short local hold so refreshes hide the slot
    try {
