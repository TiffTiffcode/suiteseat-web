...
const API_ORIGIN =
  window.location.hostname === "localhost"
    ? "http://localhost:8400"
    : "https://api2.suiteseat.io"; // ✅ PROD MUST USE api2 for cookies
...
async function checkLogin() {
  try {
    const res = await fetch(`${API_ORIGIN}/api/me?ts=${Date.now()}`, {
      credentials: "include",
      cache: "no-store",
    });
    ...
    if (res.ok && data?.user) {
      const name = displayNameFrom(data.user) || "";
      if (loginStatus) loginStatus.textContent = name ? `Hi, ${name} 👋` : "Hi 👋";
      if (logoutBtn) logoutBtn.style.display = "inline-block";
      if (openLoginBtn) openLoginBtn.style.display = "none";
      window.currentUserId = data.user._id || data.userId;
    } else {
...
logoutBtn?.addEventListener("click", async () => {
  try {
    const res = await fetch(`${API_ORIGIN}/api/logout`, {
      method: "POST",
      credentials: "include",
    });
...
async function fetchClientsWhere(whereObj, limit = 500, sortObj = { _id: -1, createdAt: -1 }) {
  ...
  const res = await apiFetch(`${apiRecords("Client")}?${qs.toString()}`);
...
async function loadAllClients(businessId = "all") {
  try {
    const where = {};
    // server-side filter optional (you can also filter client-side)
    if (businessId && businessId !== "all") {
      where["values.Business"] = businessId; // or leave empty and filter client-side
    }
    ...
    const clients = rows
      ...
      .map(r => {
        ...
        return {
          ...
          businessId: (typeof v["Business"] === "object")
            ? (v["Business"]?._id || v["Business"]?.id || "")
            : (v["Business"] || ""),
        };
      });
    ...
    console.log("[clients] loaded rows:", rows.length, "mapped clients:", clients.length);

    if (typeof renderClientList === "function") {
      renderClientList(clients);
    } else {
      console.warn("renderClientList is not defined");
    }
  } catch (err) {
...
const res = await apiFetch(`${apiRecords("Business")}?${params.toString()}`);


    if (!res.ok) throw new Error(`HTTP ${res.status}`);

const payload = await res.json();
const rawRows = Array.isArray(payload) ? payload : (payload?.items || []);
...
async function populateBusinessDropdown() {
  const dropdown = document.querySelector("#business-filter");
  if (!dropdown) return;
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/records/Business?${params.toString()}`, {
      credentials: "include",
      cache: "no-store",
    });
    ...
    // ✅ list endpoints return { items: rows } (but support array too)
    const payload = await res.json();
    const rows = Array.isArray(payload) ? payload : (payload?.items || []);
...
async function populateClientBusinessDropdown() {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE}/api/records/Business?${params.toString()}`,
      {
        credentials: "include",
        cache: "no-store",
      }
    ...
    console.log(`✅ Populated #client-business with ${cleaned.length} businesses`);
  } catch (err) {
...
async function detectClientFieldMap() {
  const url =
    `${apiRecords('Client')}?limit=5&sort=${encodeURIComponent(
      JSON.stringify({ _id: -1, createdAt: -1 })
    )}&ts=${Date.now()}`;

// ✅ Generic fetch by url (was: fetch(url, { credentials... }))
...
async function loadBusinessOptions() {
  ...
  try {
    ...
    // ✅ FIX: handle { items: [...] } payload shape
    const payload = await res.json();
    const rows = Array.isArray(payload) ? payload : (payload?.items || []);
 ...
 async function ensureClientBusinessOptions(preferredId = "") {
  ...
  try {
    ...
    if (filterHasOptions) {
      ...
      console.log(`✅ Loaded ${cleaned.length} businesses from API`);
    }
...
async function populateBusinessSelect(selectEl, preferredId = "") {
  ...
  try {
    ...
    console.log(`✅ Populated business select with ${cleaned.length} businesses`);
  } catch (err) {
