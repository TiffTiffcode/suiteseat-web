...
// ======================================================
// Business popup open + hero preview + helpers (CLEAN)
// Paste BELOW your new-file code (uses your apiFetch + uploadOneImage)
// ======================================================

// Ensure these globals exist (safe even if already defined elsewhere)
window.businessCache = window.businessCache || new Map();
window.__bizUiInitDone = window.__bizUiInitDone || false;

// track selected business for update/delete later
window.editingBusinessId = window.editingBusinessId || null;

// ---------------
// URL helper for hero image (if you store /uploads/... or full https url)
// ---------------
function toUrl(v) {
  if (!v) return "";
...
function setHeroPreview(src) {
  ...
  const finalSrc = src ? toUrl(src) : "";
  if (img) {
    img.src = finalSrc || "";
    img.style.display = finalSrc ? "block" : "none";
  }
  if (none) none.style.display = finalSrc ? "none" : "block";
}
...
// ---------------
// Fetch Business by id (uses apiFetch from your NEW file)
// ---------------
async function fetchBusinessById(id) {
  ...
  // 1) direct endpoint
  try {
    const r = await apiFetch(`/api/records/Business/${encodeURIComponent(id)}`, {
      cache: "no-store",
    ...
    if (r.ok) {
      ...
      if (data) window.businessCache?.set?.(id, data);
      return data;
    } else {
      console.warn("[fetchBusinessById] HTTP", r.status);
    }
  } catch (err) {
    console.error("[fetchBusinessById] error", err);
  }

  // 2) fallback list endpoint (only if your server still supports it)
  try {
    const r = await apiFetch(`/get-records/Business`, {
      cache: "no-store",
      headers: { Accept: "application/json" },
    });
    ...
    if (r.ok) {
      ...
      console.warn("[fetchBusinessById] fallback /get-records/Business HTTP", r.status);
    }
  } catch (err) {
    console.error("[fetchBusinessById] fallback error", err);
  }
...
// ---------------
// Init UI once (no duplicate listeners)
// ---------------
function initBusinessPopupUiOnce() {
  if (window.__bizUiInitDone) return;
  window.__bizUiInitDone = true;
  ...
  function openBusinessCreate() {
    ...
    // toggle create/update/delete buttons
  ...
  if (openBtn) openBtn.addEventListener("click", openBusinessCreate);

  // live preview when user selects a file
  const fileInput = document.getElementById("image-upload");
  if (fileInput) {
    fileInput.addEventListener("change", () => {
      const f = fileInput.files?.[0];
      if (f) {
        // local preview only (upload happens on save/update)
        const blobUrl = URL.createObjectURL(f);
        setHeroPreview(blobUrl);
      } else {
        setHeroPreview("");
      }
    });
...
// Run it now or after DOM loads (doesn't create duplicates)
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initBusinessPopupUiOnce);
} else {
  initBusinessPopupUiOnce();
}
// ======================================================
// Business CREATE / UPDATE / DELETE (CLEAN for NEW file)
// Uses: apiFetch, uploadOneImage, slugify, fetchMe, setHeroPreview, getCurrentHeroSrc
// ======================================================

(function bindBusinessCrudOnce() {
  if (window.__bizCrudBound) return;
  window.__bizCrudBound = true;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!values.businessName) return alert("Business Name is required");

      // ✅ HERO IMAGE (CREATE)
      let heroUrl = getCurrentHeroSrc();
      if (fileInput?.files?.[0]) {
        heroUrl = await uploadOneImage(fileInput.files[0]); // ✅ uses your NEW helper
      }
      if (heroUrl) {
        values.heroImageUrl = heroUrl;
        values.heroImage = heroUrl; // optional: keep both if your UI expects it
      }

      // ✅ slug (use NEW file's slugify)
      const slug = slugify(values.businessName);
      values.slug = slug;
      values.businessSlug = slug;
      values.bookingSlug = slug;

      // ✅ proUserId (use your existing /api/me helper)
      let proUserId = null;
      try {
        const meData = await fetchMe(); // already uses apiFetch("/api/me")
      ...
      // ✅ POST create
      const res = await apiFetch(`/api/records/${encodeURIComponent(TYPE_NAME)}`, {
        method: "POST",
      ...
      // refresh lists (only if they exist)
      ...
      // allow closing forced popup state if you use it
      if (typeof REQUIRE_FIRST_BUSINESS !== "undefined") REQUIRE_FIRST_BUSINESS = false;
  ...
  if (updateBtn && !updateBtn.dataset.bound) {
    updateBtn.addEventListener("click", async () => {
      const editingId = window.editingBusinessId;
      if (!editingId) return alert("No business selected.");
      ...
      let heroUrl = getCurrentHeroSrc();
      const f = fileInput?.files?.[0];
      if (f) heroUrl = await uploadOneImage(f);
      ...
      // ✅ keep slug updated (based on name)
      ...
      try {
        const res = await apiFetch(
          `/api/records/${encodeURIComponent(TYPE)}/${encodeURIComponent(editingId)}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json", Accept: "application/json" },
            body: JSON.stringify({ values }),
        ...
        // cache + preview
        window.businessCache?.set?.(editingId, updated);
        ...
        await loadBusinessDropdown?.({ preserve: true, selectId: editingId });
        await loadBusinessList?.();
      } catch (e) {
        console.error(e);
  ...
  if (deleteBtn && !deleteBtn.dataset.bound) {
    deleteBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (saved === editingId) sessionStorage.removeItem("selectedBusinessId");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// Calendar Section (NEW FILE COMPATIBLE - uses apiFetch)
// Requires: apiFetch, closeAllPopups (optional), loadCalendarBusinessOptions, loadCalendarList, closeAddCalendarPopup
///////////////////////////////////////////////////////////////////////////////////////////////////////////

window.editingCalendarId = window.editingCalendarId || null;
...
async function openCalendarCreate() {
  window.editingCalendarId = null; // ✅ create mode
  ...
  if (typeof loadCalendarBusinessOptions === "function") {
...
// overlay close (only once)
if (overlay && !overlay.dataset.calendarBound) {
  overlay.addEventListener("click", () => {
    if (typeof closeAllPopups === "function") closeAllPopups();
  });
  overlay.dataset.calendarBound = "1";
}

// =========================
// ✅ Save Calendar (NEW FILE COMPATIBLE)
// =========================
...
if (saveCalBtn && calNameInput && calBizSelect && !saveCalBtn.dataset.bound) {
  saveCalBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (typeof loadCalendarList === "function") await loadCalendarList();
      if (typeof closeAddCalendarPopup === "function") closeAddCalendarPopup();
    } catch (e) {
      console.error(e);
...
///////////////////////////////////////////////////////////////////
// Category Section (NEW FILE COMPATIBLE - uses apiFetch)
///////////////////////////////////////////////////////////////////
...
if (openCategoryBtn && categoryPopup && overlay && !openCategoryBtn.dataset.bound) {
  openCategoryBtn.addEventListener("click", async () => {
    try {
      // 1) Fill business dropdown inside popup
      ...
      // 3) Load calendars for selected business
      if (typeof loadCalendarOptions === "function") {
      ...
      console.error("[category] open popup error:", err);
      alert("Could not open category popup.");
    }
  });

  openCategoryBtn.dataset.bound = "1";
}

// Bind the CHANGE handler once (outside the click handler)
...
if (popupBizSel && !popupBizSel.dataset.bound) {
  popupBizSel.addEventListener("change", async () => {
    try {
      ...
      console.error("[category] business change error:", err);
    }
  });
  popupBizSel.dataset.bound = "1"; // prevent double-binding
}

// Optional: overlay click closes category popup (bind once)
if (overlay && !overlay.dataset.categoryCloseBound) {
  overlay.addEventListener("click", () => {
    if (typeof closeCategoryPopup === "function") closeCategoryPopup();
  });
  overlay.dataset.categoryCloseBound = "1";
}

// =========================
// ✅ Save Category (NEW FILE COMPATIBLE)
// =========================
const saveCategoryBtn = document.getElementById("save-category-button");
const catNameInput    = document.getElementById("popup-category-name-input");
...
if (saveCategoryBtn && catBizSelect && catCalSelect && !saveCategoryBtn.dataset.bound) {
  saveCategoryBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (typeof loadCategoryList === "function") await loadCategoryList();
      if (typeof closeCategoryPopup === "function") closeCategoryPopup();
    } catch (e) {
      console.error(e);
  ...
  saveCategoryBtn.dataset.bound = "1";
}
...
///////////////////////////////////////////////////////////////////
// Service Section (NEW FILE COMPATIBLE - uses apiFetch)
///////////////////////////////////////////////////////////////////

// Open Add Service Popup (create mode)
...
if (openServiceBtn && !openServiceBtn.dataset.bound) {
  openServiceBtn.addEventListener("click", () => {
    openServiceCreate().catch(console.error);
  });
...
if (svcBizSel && !svcBizSel.dataset.bound) {
  svcBizSel.addEventListener("change", async () => {
    try {
      ...
      console.error("[service] business change error:", err);
    }
  });
...
if (svcCalSel && !svcCalSel.dataset.bound) {
  svcCalSel.addEventListener("change", async () => {
    try {
      ...
      console.error("[service] calendar change error:", err);
    }
  });
  svcCalSel.dataset.bound = "1";
}

// ===== Save Service (submit form) =====
...
if (serviceForm && !serviceForm.dataset.bound) {
  serviceForm.addEventListener("submit", async (e) => {
    ...
    const durationRaw     = durSel?.value || "";
    ...
    if (!durationRaw)  return alert("Please choose a duration.");

    const durationMinutes = parseInt(durationRaw, 10);
    if (!Number.isFinite(durationMinutes) || durationMinutes <= 0) {
      return alert("Duration is invalid.");
    }
    ...
    try {
      // 1) Optional image upload (uses apiFetch)
      let imageUrl = "";
      ...
      if (file) {
        ...
        if (!up.ok) {
          const msg = upJson?.error || upJson?.message || `HTTP ${up.status}`;
      ...
      const res = await apiFetch(`/api/records/${encodeURIComponent(TYPE)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ values }),
      });
...
// Preview service image (bind once)
...
/////////////////////////////////////////////
// End Menu Section (NEW FILE COMPATIBLE)
/////////////////////////////////////////////

// Close All Popups
function closeAllPopups() {
  // Hide all popups that share this class
  document.querySelectorAll(".popup-add-business").forEach((el) => {
    el.style.display = "none";
  });

  // Also hide login popup(s) depending on markup
  const loginA = document.getElementById("popup-login");
  if (loginA) loginA.style.display = "none";

  const loginB = document.getElementById("login-popup");
...
// ⚠️ IMPORTANT:
// You already have closeLoginPopup() in your NEW file.
// Do NOT redeclare it here, or you'll create duplicates.
// If other code calls closeLoginPopup(), it will still work.
...
// NEW FILE version: uses apiFetch, not apiUrl
...
/////////////////////////////////////////////
// End Business Section (NEW FILE COMPATIBLE)
/////////////////////////////////////////////

// ⚠️ You ALREADY defined closeAddBusinessPopup() earlier (from the End Menu section).
// Do NOT redefine it here or you’ll create duplicates.
// If you want this behavior, DELETE the older one and keep this one.
...
function refId(v) {
  if (!v) return "";
  if (typeof v === "string") return v;
  if (typeof v === "object") return v._id || v.id || v.value || v.$id || "";
  return "";
}
function slugForBiz(biz) {
  ...
  return (
    ...
    getFirst(v, ["businessName", "name", "Business Name"])
      ?.toString()
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
...
// ✅ NEW FILE: publicList uses apiFetch (no apiUrl)
async function publicList(dataType, filters = {}, limit = 500) {
  ...
  const r = await apiFetch(`/public/records?${params.toString()}`, {
    method: "GET",
    credentials: "include",
    headers: { Accept: "application/json" },
    cache: "no-store",
...
function isCanceledAppt(v) {
  ...
  const status = (v["Appointment Status"] || v["Status"] || v.status || "")
    .toString()
    .toLowerCase();
  return ["canceled", "cancelled", "void", "no show", "no-show"].includes(status);
}

// --- unique clients count from appointments ---
async function fillClientCount(businessId, targetEl) {
  try {
    let appts = await publicList("Appointment", { Business: businessId });
    if (!appts.length) appts = await publicList("Appointment");

    const uniq = new Set();

    const addClient = (val, v) => {
      if (Array.isArray(val)) return val.forEach((x) => addClient(x, v));

      const id = refId(val);
      if (id) return uniq.add(String(id));
    ...
    for (const r of appts) {
      ...
      if (candidates.length) candidates.forEach((cv) => addClient(cv, v));
      else {
        const email = v["Client Email"] || v.clientEmail || v.Email || v.email;
        if (email) uniq.add(`email:${String(email).toLowerCase()}`);
      }
...
function openBusinessEdit(biz) {
  ...
  // ✅ safe setters (no more "(el||{}).value =" which throws)
  const setVal = (id, val) => {
    const el = document.getElementById(id);
    if (el) el.value = val ?? "";
  };

  setVal("popup-business-name-input",          v.businessName || v.name || "");
  setVal("popup-your-name-input",              v.yourName || "");
  setVal("popup-business-phone-number-input",  v.phoneNumber || "");
  setVal("popup-business-location-name-input", v.locationName || "");
  setVal("popup-business-address-input",       v.businessAddress || "");
  setVal("popup-business-email-input",         v.businessEmail || "");

  const heroRaw = v.heroImageUrl ?? v.heroImage ?? v["Hero Image"] ?? v.hero_image ?? "";
  const heroUrl = (typeof toUrl === "function") ? toUrl(heroRaw) : (heroRaw || "");
...
/////////////////////////////////////////////
// End Calendar Section (NEW FILE COMPATIBLE)
/////////////////////////////////////////////
...
async function loadCalendarList() {
  ...
  try {
    // ✅ NEW FILE: use apiFetch
    const res = await apiFetch(`/api/records/Calendar?ts=${Date.now()}`, {
      method: "GET",
      cache: "no-store",
      headers: { Accept: "application/json" },
    });
    ...
    rows.forEach((row) => {
      ...
      radio.addEventListener("change", async () => {
        ...
        try {
          // helper to PATCH with good errors (✅ NEW FILE: uses apiFetch)
          async function patchCal(id, values) {
            const r = await apiFetch(`/api/records/Calendar/${encodeURIComponent(id)}`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json", Accept: "application/json" },
              body: JSON.stringify({ values }),
            });
...
/////////////////////////////////////////////
// End Category Section (NEW FILE COMPATIBLE)
/////////////////////////////////////////////
...
async function loadBusinessOptions(selectId) {
  ...
  try {
    ...
    const payload = await res.json().catch(() => ({}));
    const businesses = (Array.isArray(payload?.items) ? payload.items : []).filter(
      (b) => !b?.deletedAt
    );
    ...
    businesses.forEach((biz) => {
      ...
      opt.value = biz._id;
      opt.textContent =
        biz?.values?.businessName || biz?.values?.Name || "(Untitled)";
      sel.appendChild(opt);
    });
...
async function loadCalendarOptions(selectId, businessId) {
  ...
  try {
    ...
    const rows = all.filter((c) => !c?.deletedAt && sameBusiness(c.values || {}, businessId));
...
async function loadCategoryOptions(selectId, businessId, calendarId) {
  ...
  try {
    const res = await apiFetch(`/api/records/Category?ts=${Date.now()}`, {
      method: "GET",
      cache: "no-store",
      headers: { Accept: "application/json" },
    });
    ...
    const categories = raw
      ...
      .filter((c) => {
        ...
        const cid = String(
          firstDefined(v.calendarId, asId(v.Calendar), asId(v["Calendar"])) || ""
        );
    ...
    categories.forEach((cat) => {
      const v = cat.values || {};
      const label = firstDefined(v.categoryName, v.Name, v.name, "(Untitled)");
...
// UPDATE/DELETE handlers (bind once)
(function bindCalendarUpdateDeleteOnce() {
  ...
  if (updateBtn && !updateBtn.dataset.bound) {
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await apiFetch(
          `/api/records/${encodeURIComponent(TYPE)}/${encodeURIComponent(editingCalendarId)}`,
          {
            method: "PATCH",
            headers: {
...
async function loadCategoryList() {
  ...
  try {
    const [catRes, calRes] = await Promise.all([
      apiFetch(`/api/records/Category?ts=${Date.now()}`, {
        method: "GET",
        cache: "no-store",
        headers: { Accept: "application/json" },
      }),
      apiFetch(`/api/records/Calendar?ts=${Date.now()}`, {
        method: "GET",
        cache: "no-store",
        headers: { Accept: "application/json" },
      }),
    ...
    const [catPayload, calPayload] = await Promise.all([
      catRes.json().catch(() => ({})),
      calRes.json().catch(() => ({})),
    ]);

    const rawCats = Array.isArray(catPayload?.items) ? catPayload.items : [];
    const rawCals = Array.isArray(calPayload?.items) ? calPayload.items : [];

    // categories: hide soft-deleted
    const categories = rawCats.filter((c) => !c?.deletedAt);

    // calendars: only calendars for this business
    ...
    const calNameById = new Map(
      calendars.map((c) => [
        c._id,
        c?.values?.calendarName ?? c?.values?.name ?? "(Untitled)",
      ])
    );

    // Filter categories by business and optional calendar
    ...
    if (!window.categoryCache) window.categoryCache = new Map();
    ...
    rows.forEach((cat) => {
      ...
      n.textContent = v.categoryName ?? v.Name ?? v.name ?? "(Untitled)";
...
// Bind Category UI (business dropdown + calendar filter)
...
// expose if you want to call it after login/loadBusinesses
window.bindCategoryUI = bindCategoryUI;
...
async function openCategoryEdit(cat) {
  ...
  editingCategoryId = cat?._id || null;
  ...
  if (nameIn) {
    nameIn.value = v.categoryName ?? v.Name ?? v.name ?? "";
  }
...
// Update/Delete Category (bind once)
(function bindCategoryUpdateDeleteOnce() {
  ...
  if (updateBtn && !updateBtn.dataset.bound) {
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await apiFetch(
          `/api/records/${encodeURIComponent(TYPE)}/${encodeURIComponent(editingCategoryId)}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
        ...
        console.error(e);
        alert("Error updating category: " + (e?.message || e));
      } finally {
        updateBtn.disabled = false;
        updateBtn.textContent = prev;
      }
...
//////////////////////////////////////
// End Service Section (NEW FILE COMPATIBLE)
//////////////////////////////////////
...
// ✅ IMPORTANT:
// Do NOT re-declare firstDefined/asId here if you already have them above.
// This block assumes firstDefined(...) and asId(...) exist (like your Category code).
...
async function loadServiceFilterDropdown() {
  const wrapper = document.getElementById(
    "service-section-calendar-dropdown-wrapper"
  );
...
async function loadServiceList() {
  ...
  try {
    const [svcRes, calRes, catRes, myId] = await Promise.all([
      apiFetch(`/api/records/Service?ts=${Date.now()}`, {
        method: "GET",
        cache: "no-store",
        headers: { Accept: "application/json" },
      }),
    ...
    const rawServices = Array.isArray(svcPayload?.items) ? svcPayload.items : [];
    const rawCalendars = Array.isArray(calPayload?.items)
      ? calPayload.items
      : [];
    const rawCategories = Array.isArray(catPayload?.items)
      ? catPayload.items
      : [];
    ...
    const services = rawServices
      ...
      .filter((s) => {
        // OPTIONAL: can remove; keep if your /api/records returns more than user scope
    ...
    if (!nameCol.dataset.bound) {
      nameCol.addEventListener("click", (e) => {
        ...
        if (svc) window.openServiceEdit?.(svc);
      });
      nameCol.dataset.bound = "1";
    }
  } catch (e) {
  ...
  if (updateBtn && !updateBtn.dataset.bound) {
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const TYPE = "Service";
        const res = await apiFetch(
          `/api/records/${encodeURIComponent(TYPE)}/${encodeURIComponent(
            editingServiceId
          )}`,
          {
            method: "PATCH",
...
// Auto-resize description textarea (keep ONE DOMContentLoaded in your whole file)
// If your new file already has a DOMContentLoaded block, move this inside it.
function bindServiceDescAutosize() {
...
window.openServiceEdit = async function openServiceEdit(svc) {
  ...
  if (durSel) durSel.value = String(svc?.values?.durationMinutes ?? "");
...
// expose so you can call it after loadBusinessDropdown()
window.bindServiceUI = bindServiceUI;
window.bindServiceDescAutosize = bindServiceDescAutosize;
