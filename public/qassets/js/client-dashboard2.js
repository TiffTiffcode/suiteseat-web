console.log('[client-dashboard2] web loaded');

const API = window.NEXT_PUBLIC_API_ORIGIN || "http://localhost:8400";
    ...
    if (data.loggedIn && headerRight) {
      ...
      document.getElementById("logout-btn")?.addEventListener("click", async () => {
        // use the route your server actually exposes
         const resLogout = await fetch(`${API}/auth/logout`, { method: "POST", credentials: "include" });
        if (resLogout.ok) {
          location.reload();
        } else {
