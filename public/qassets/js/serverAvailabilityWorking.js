...
const recordsCtrl = require('./controllers/records.js');  // <-- note the .js
...
app.use(session({
  ...
  store: MongoStore.create({ mongoUrl: process.env.MONGODB_URI, ttl: 60*60*24*30 }),
  cookie: {
    httpOnly: true,
    sameSite: 'lax',     // ✅ works for same-origin & Next.js rewrite
    secure: isProd,      // ✅ false on localhost
    domain: isProd ? '.suiteseat.io' : undefined, // ✅ no domain in dev
...
app.post('/api/records', requireLogin, recordsCtrl.createRecord);
...
app.post('/api/records', async (req, res) => {
  try {
    const { dataTypeId, values = {}, businessId, createdBy } = req.body || {};
      const uid = req.session?.userId || createdBy;
    const userId =
  ...
  console.log('\n🟩 [POST /api/records]');
    ...
    if (!userId) {
      return res.status(401).json({ error: 'Login required (no createdBy available)' });
    }
...
app.post('/api/records', requireLogin, async (req, res) => {
  try {
    ...
    if (!dataTypeId) {
      return res.status(400).json({ error: 'MISSING_DATATYPE', message: 'dataTypeId is required' });
    }
    const uid = req.session.userId;
    const doc = await Record.create({ dataTypeId, values, createdBy: uid });
